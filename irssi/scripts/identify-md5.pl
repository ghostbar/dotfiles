...
my %nicknames = (
	lc('ghostbar')		=> 'n0l33t', 		# Plain text password 'password1'
...
sub cmd_identify {
    ...
    $server->command("QUOTE NickServ identify ghostbar n0l33t") if $#identify >= 0;
}
...
sub event_notice {
	...
	if($text =~ /^205 S\/MD5 1\.0 (.+)$/ && $#identify >= 0) {
	    ...
	    $server->command("QUOTE NickServ identify $nickname $hash");

	    # Suppress the notice from NickServ
	    Irssi::signal_stop();

	    # And get a new cookie if there are still nicks left to identify to
#	    $server->command("QUOTE NickServ identify") if $#identify >= 0;
	}
