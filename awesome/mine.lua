...
theme_path = "/home/ghostbar/.config/awesome/default"
...
--- taked from http://silenceisdefeat.org/~koniu/awesome/rc.lua
configwidgetsspace = "   "
...
function widgettext(label, value, labelcolor, valuecolor)
	...
	return 	'<span color="' .. lc .. '">' .. label .. ' </span><span color="' .. vc .. '">'  .. value .. '</span>' .. configwidgetsspace
end
...
local function get_bat2()
	local color = ''
	local v = ''
	local a = io.open('/sys/class/power_supply/BAT2/status')
	local status = a:read()
	a:close()

	if 	status == "Full" 	then v = widgettext('BAT2', '100%',nil,nil); 
	else
	
		local a = io.open('/sys/class/power_supply/BAT2/energy_full')
		local full = a:read()
		a:close()
		local a = io.open('/sys/class/power_supply/BAT2/energy_now')
		local now = a:read()
		a:close() 
		...
		v = widgettext('BAT2', bat .. '%',nil,color)
	end
...
-- switching tabs
keybinding({}, "#234", awful.tag.viewprev):add()
keybinding({}, "#233", awful.tag.viewnext):add()
keybinding({ modkey }, "Escape", awful.tag.history.restore):add()

volup="amixer sset Master,0 1+"
voldown="amixer sset Master,0 1-"
volmute="amixer sset Master,0 toggle"
-- sound buttons
keybinding({}, "#176", function () awful.spawn(volup) end):add()
keybinding({}, "#174", function () awful.spawn(voldown) end):add()
keybinding({}, "#160", function () awful.spawn(volmute) end):add()

scrot="scrot ~/tmp/$(date +%Y%m%d-%k%M%S).png"
-- printing screen
keybinding({}, "Print", function () awful.spawn(scrot) end):add()
